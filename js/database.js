...
function iniciarDadosDemo(){
  if (dbGet(DB_KEYS.usuarios).length > 0) return;
...
function autenticar(email, senha, tipoEsperado){
  return dbGet(DB_KEYS.usuarios).find(u => u.email.toLowerCase() === email.toLowerCase() && u.senha === senha && u.tipo === tipoEsperado) || null;
}
function criarUsuario(usuario){
  const usuarios = dbGet(DB_KEYS.usuarios);
  if (usuarios.some(u => u.email.toLowerCase() === usuario.email.toLowerCase())) throw new Error("Já existe usuário com este e-mail.");
  const novo = {id:gerarId(usuario.tipo),criadoEm:hojeISO(),foto:"",...usuario};
  usuarios.push(novo); dbSet(DB_KEYS.usuarios, usuarios); return novo;
}
function atualizarUsuario(id, novosDados){
  ...
  if (emailNovo && usuarios.some(u => u.id !== id && u.email.toLowerCase() === emailNovo)) throw new Error("Este e-mail já está em uso.");
...
function listarAlunosDoProfessor(professorId){ return dbGet(DB_KEYS.usuarios).filter(u => u.tipo === "aluno" && u.professorId === professorId).sort((a,b)=>a.nome.localeCompare(b.nome)); }
function salvarOuAtualizarTreino({ professorId, alunoId, diaSemana, nome }){
  ...
  dbSet(DB_KEYS.treinos, treinos); return treino;
}
...
function criarExercicio(exercicio){ const exercicios=dbGet(DB_KEYS.exercicios); const novo={id:gerarId("ex"),criadoEm:hojeISO(),...exercicio}; exercicios.push(novo); dbSet(DB_KEYS.exercicios,exercicios); return novo; }
...
function alternarExecucao({ professorId, alunoId, treinoId, exercicioId }){
  const execucoes=dbGet(DB_KEYS.execucoes); const dataExecucao=hojeISO();
...
function resetarDadosLocais(){ Object.values(DB_KEYS).forEach(k=>localStorage.removeItem(k)); limparSessao(); iniciarDadosDemo(); }
