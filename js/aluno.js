...
function prepararBottomNav() {
  document.querySelectorAll(".bottom-link").forEach(btn => {
    btn.addEventListener("click", () => {
      document.querySelectorAll(".bottom-link").forEach(b => b.classList.remove("active"));
      document.querySelectorAll(".student-page").forEach(p => p.classList.remove("active"));
      btn.classList.add("active");
      document.getElementById("student-" + btn.dataset.studentPage).classList.add("active");
      carregarTudoAluno();
    });
  });
}
...
function preencherPerfilAluno() {
  ...
  const avatar = document.getElementById("alunoAvatarPreview");
  avatar.textContent = inicialNome(aluno.nome);
}
...
function carregarTreinoHoje() {
  ...
  box.innerHTML = exerciciosHoje.map(ex => {
    ...
    return `
      <article class="exercise-item ${feito ? "done" : ""}">
        <header>
          <div>
            <strong>${ex.nome}</strong>
            <div class="meta">${ex.series || 0} séries • ${ex.repeticoes || "-"} repetições${ex.carga ? " • Carga: " + ex.carga : ""}${ex.descanso ? " • Descanso: " + ex.descanso : ""}</div>
            ${ex.observacao ? `<div class="meta">Obs: ${ex.observacao}</div>` : ""}
          </div>
          <span class="badge ${feito ? "" : "pending"}">${feito ? "Feito" : "Pendente"}</span>
        </header>
        <div class="action-row">
          <button class="small-btn ${feito ? "" : "success"}" onclick="marcarExercicio('${ex.id}')">${feito ? "Desmarcar" : "Marcar como feito"}</button>
        </div>
      </article>
    `;
  }).join("");
...
function carregarSemana() {
  ...
  if (!treinos.length) {
    box.innerHTML = `<p class="meta">Nenhum treino semanal cadastrado.</p>`;
    return;
  }
  ...
  box.innerHTML = dias.map(dia => {
    ...
    if (!treino) {
      return `<div class="week-item"><div><strong>${diaSemanaNome(dia)}</strong><div class="meta">Sem treino cadastrado</div></div><span class="badge neutral">Livre</span></div>`;
    }
...
function carregarHistorico() {
  ...
  if (!execucoes.length) {
    box.innerHTML = `<p class="meta">Nenhum exercício concluído ainda.</p>`;
    return;
  }

  const agrupado = {};
  execucoes.forEach(e => {
    agrupado[e.dataExecucao] = (agrupado[e.dataExecucao] || 0) + 1;
  });

  box.innerHTML = Object.entries(agrupado)
    .sort((a,b) => b[0].localeCompare(a[0]))
    .map(([data, qtd]) => `<div class="history-item"><div><strong>${data}</strong><div class="meta">${qtd} exercício(s) concluído(s)</div></div><span class="badge">Feito</span></div>`)
    .join("");
}
