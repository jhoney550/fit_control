...
function login() {
  try {
    if (!email.value.trim() || !senha.value) return setMsg("Informe e-mail e senha.", "error");
    const usuario = autenticar(email.value.trim(), senha.value, tipoLoginSelecionado);
    if (!usuario) return setMsg("Login inválido para a aba selecionada.", "error");
    setSessao(usuario);
