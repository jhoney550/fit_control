...
function iniciarProfessor() {
  ...
  document.getElementById("professorNomeMobile").textContent = professor.nome;
...
function prepararSubtabs() {
  document.querySelectorAll(".subtab").forEach(btn => {
    btn.addEventListener("click", () => {
      document.querySelectorAll(".subtab").forEach(b => b.classList.remove("active"));
      document.querySelectorAll(".subpage").forEach(p => p.classList.remove("active"));
      btn.classList.add("active");
      document.getElementById("sub-" + btn.dataset.subpage).classList.add("active");
    });
  });
}
...
function preencherListaExercicios() {
  document.getElementById("listaExercicios").innerHTML =
    EXERCICIOS_SUGERIDOS.map(e => `<option value="${e}"></option>`).join("");
}
...
function preencherPerfilProfessor() {
  ...
  if (professor.foto) {
    avatar.style.backgroundImage = `url('${professor.foto}')`;
    avatar.textContent = "";
  }
}
...
function salvarPerfilProfessor() {
  try {
    ...
    if (!dados.nome || !dados.email) {
      setMsg("Nome e e-mail são obrigatórios.", "error");
      return;
    }
...
function cadastrarAluno() {
  try {
    ...
    if (!nome || !email || !senha) {
      setMsg("Nome, e-mail e senha são obrigatórios.", "error");
      return;
    }
    if (senha.length < 6) {
...
function carregarDashboard() {
  ...
  if (!alunos.length) {
    box.innerHTML = `<p class="meta">Nenhum aluno cadastrado.</p>`;
    return;
  }
  box.innerHTML = alunos.slice(0, 5).map(a => {
      ...
      <article class="student-item">
        <header>
          <div>
            ...
            <div class="meta">${aluno.email} ${aluno.whatsapp ? " • " + aluno.whatsapp : ""}</div>
            <div class="meta">Objetivo: ${aluno.objetivo || "-"}${aluno.peso ? " • " + aluno.peso + "kg" : ""}${aluno.altura ? " • " + aluno.altura + "cm" : ""}</div>
          </div>
          <span class="badge">${pct}% hoje</span>
        </header>
        <div class="action-row">
          <button class="small-btn warning" onclick="abrirMontarTreino('${aluno.id}')">Montar treino</button>
          <button class="small-btn" onclick="abrirProgresso()">Ver progresso</button>
        </div>
      </article>
    `;
  }).join("");
...
function abrirMontarTreino(alunoId) {
  ...
  setTimeout(() => {
    document.getElementById("selectAluno").value = alunoId;
    atualizarTreinoAlunoSelecionado();
  }, 0);
}

function abrirProgresso() {
  document.querySelector('[data-page="progresso"]').click();
}
...
function salvarExercicioNoTreino() {
  try {
    ...
    if (!alunoId || !nomeTreino || !nomeExercicio) {
      setMsg("Selecione aluno, nome do treino e exercício.", "error");
      return;
    }
...
function atualizarTreinoAlunoSelecionado() {
  ...
  if (!treinos.length) {
    box.innerHTML = `<p class="meta">Nenhum treino cadastrado para este aluno.</p>`;
    return;
  }
      ...
      <div class="student-item">
        <header>
          <div>
            ...
            <div class="meta">Hoje: ${p.concluidosHoje} de ${p.totalExercicios} exercícios concluídos.</div>
          </div>
          <span class="badge">${pct}%</span>
        </header>
        <div class="progress-bar" style="margin-top:12px"><div style="width:${pct}%"></div></div>
      </div>
    `;
  }).join("");
}
